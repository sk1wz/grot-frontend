...
const moduleIcons: Record<CheckModule, string> = {
  [CheckModule.GIBDD]: "Icongibdd",
  [CheckModule.GISTORGI]: "Icontorgi",
  [CheckModule.LIMITATION]: "Iconlocks",
  [CheckModule.FSSP]: "Iconfssp",
  [CheckModule.BANKRUPTCY]: "Iconbankcrupcy",
  [CheckModule.INN]: "Iconinn",
  [CheckModule.TAXI]: "Icontaxi",
};
...
export function TariffsPageContent() {
  ...
  const orderedPrices = useMemo(
    () =>
      moduleOrder.flatMap((module) =>
        prices.filter((price) => price.module === module)
      ),
    [prices]
  );
  ...
  return (
    <DashboardPageFrame figureSrc="/images/tariff.png">
      <h1 className="mb-[50px] text-[32px] leading-none font-medium text-(--foreground) max-[700px]:mb-8 max-[700px]:text-[28px]">
        Тарифы
      </h1>
      <section
      ...
      >
        ...
        {isLoading ? (
          <p className="px-10 py-8 text-center text-sm text-[#868a85]">
            Загрузка тарифов…
          </p>
        ) : error ? (
          <p className="px-10 py-8 text-center text-sm text-[#b14b4b]">
            {error}
          </p>
        ) : (
          <ul className="m-0 list-none p-0">
            {orderedPrices.map((tariff) => (
              <li
              ...
              >
                <Image
                  src={`/images/tariff-icons/${moduleIcons[tariff.module]}.svg`}
                  width={40}
                  height={40}
                  alt=""
                ...
                <span className="ml-[14px] text-[18px] leading-6 font-semibold text-[#3e3c4b] uppercase max-[700px]:ml-2.5 max-[700px]:text-[13px] max-[700px]:leading-[18px]">
                  {tariff.description || tariff.title}
                </span>
                <strong className="text-right text-[24px] font-medium text-[#3e3c4b] max-[700px]:text-[16px]">
